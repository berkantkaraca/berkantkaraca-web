    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Sertifika Yönetimi') }}
            </h2>
            <a href="{{ route('admin.certificates.create') }}" class="btn btn-outline-dark">
                Yeni Sertifika Ekle
            </a>
        </div>
    </x-slot>
                        ...
                        <table class="min-w-full bg-white border border-gray-300">
                            <thead>
                                <tr class="bg-gray-100">
                                    ...
                                    <th class="py-2 px-4 border-b text-left">Veren Kurum</th>
                                    <th class="py-2 px-4 border-b text-left">Veriliş Tarihi</th>
                            ...
                            <tbody>
                                @forelse($certificates as $certificate)
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-2 px-4 border-b">{{ $certificate->id }}</td>
                                        <td class="py-2 px-4 border-b">{{ $certificate->title }}</td>
                                        <td class="py-2 px-4 border-b">{{ $certificate->issuer }}</td>
                                        <td class="py-2 px-4 border-b">{{ $certificate->issue_date->format('d.m.Y') }}</td>
                                        <td class="py-2 px-4 border-b">{{ $certificate->sort_order }}</td>
                                        ...
                                        <td class="py-2 px-4 border-b">
                                            <div class="flex items-center space-x-2">
                                                <a href="{{ route('admin.certificates.show', $certificate) }}" class="text-blue-600 hover:text-blue-800">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('admin.certificates.edit', $certificate) }}" class="text-yellow-600 hover:text-yellow-800">
