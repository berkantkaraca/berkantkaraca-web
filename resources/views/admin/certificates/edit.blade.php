            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.certificates.update', $certificate) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="image" class="block text-gray-700 text-sm font-bold mb-2">Sertifika Resmi:</label>
                            @if($certificate->image_path)
                                <div class="mb-2">
                                    <img src="{{ $certificate->image_url }}" alt="{{ $certificate->title }}" class="max-w-xs rounded-lg shadow-md">
                                    <p class="text-sm text-gray-600 mt-1">Mevcut resim</p>
                                </div>
                            @endif
                            <input type="file" name="image" id="image" accept="image/*"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            <small class="text-gray-600">Yeni resim seçmezseniz mevcut resim korunur. Desteklenen formatlar: JPEG, PNG, JPG, GIF, WEBP (Max: 2MB)</small>
                        </div>

                        <div class="mb-4">
                            <label for="description" class="block text-gray-700 text-sm font-bold mb-2">Açıklama (Opsiyonel):</label>
                            <textarea name="description" id="description" rows="4"
                                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">{{ old('description', $certificate->description) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                            <div>
                                <label for="sort_order" class="block text-gray-700 text-sm font-bold mb-2">Sıra:</label>
                                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $certificate->sort_order) }}" min="0"
                                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="is_active" id="is_active" 
                                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                    <option value="1" {{ old('is_active', $certificate->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('is_active', $certificate->is_active) == '0' ? 'selected' : '' }}>Pasif</option>
                                </select>
                            </div>
                        </div>
