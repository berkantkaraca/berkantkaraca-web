      ...
      <div class="col-md-12">
        @if(isset($portfolios) && $portfolios->count() > 0)
          @foreach($portfolios as $index => $portfolio)
            <div class="card mb-3" data-aos="fade-right" data-aos-delay="{{ ($index + 1) * 100 }}">
              <div class="card-header px-3 py-2">
                <div class="d-flex justify-content-between">
                  <div>
                    <h3 class="h5 mb-1">{{ $portfolio->title }}</h3>
                    <div class="text-muted text-small">{{ $portfolio->technology }}</div>
                  </div>
                </div>
              </div>
              <div class="card-body px-3 py-2">
                ...
                @if($portfolio->features)
                  <p><strong>Temel Özellikler:</strong> {{ $portfolio->features }}</p>
                @endif

                @if($portfolio->security_features)
                  <p><strong>Güvenlik Özellikleri:</strong> {{ $portfolio->security_features }}</p>
                @endif

                @if($portfolio->microservices)
                  <p>Mikroservis Yapısı:</p>
                  <ul>
                    @foreach(explode("\n", $portfolio->microservices) as $service)
                      @if(trim($service))
                        <li>{!! str_replace('=>', ' => ', trim($service)) !!}</li>
                      @endif
                    @endforeach
                  </ul>
                @endif
                ...
                @if($portfolio->technologies_used)
                  ...
                  <ul>
                    @foreach(explode("\n", $portfolio->technologies_used) as $tech)
                      @if(trim($tech))
                        <li>{!! trim($tech) !!}</li>
                      @endif
                    @endforeach
                ...
                @if($portfolio->github_url)
                  <a href="{{ $portfolio->github_url }}" class="btn btn-primary" target="_blank">Projeyi incele</a>
                @endif
                
                @if($portfolio->demo_url)
                  <a href="{{ $portfolio->demo_url }}" class="btn btn-outline-primary ml-2" target="_blank">Demo</a>
                @endif
              </div>
            </div>
          @endforeach
